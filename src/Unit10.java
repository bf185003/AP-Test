...
public class Unit10 {
    ...
    //2:必须要有退出机制，否则死循环了。
    //递归和循环类似，优点是递归代码比较简洁，
    // 缺点是：1难以理解，2 比较消耗内存，function1做到10000就内存溢出了
    //大多数递归都能用循环语句替换，比如用for和while也能做阶乘。
    //另外，递归能做树和图的遍历
    ...
    //繁殖兔子例子，或者繁殖细菌
    //早期教学：用兔子繁殖模型强调递归关系，更适合引入斐波那契数列的基本公式。必须强调“第一对兔子从第二个月才开始繁殖”，容易被忽略。
    //另外兔子的单位是“对”，不是个，不容易直观地从单个兔子数量来看。
    //高级教学：结合细菌繁殖展示快速增长，帮助学生理解递归算法的效率和实际应用，例如在大数据或模拟环境中的使用。
    //用递归计算斐波那契数列的时间复杂度是O（2^n），因为会重复计算算子，导致指数级增长，因此，实际项目中不要用双递归，可以用循环来实现。
    ...
    public long Fibonacci(long index) {
        if (index == 0)
            ...
            System.out.println("I am " + index);//可以尝试一下看到底2和3调用了多少次
